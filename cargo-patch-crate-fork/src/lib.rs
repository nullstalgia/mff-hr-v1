...
fn copy_package(pkg: &Package, patch_target_folder: &Path, overwrite: bool) -> Result<PathBuf> {
    ...
    copy_dir_all(pkg.root(), patch_target_folder)?;
    Ok(patch_target_path)
}
...
pub fn run() -> anyhow::Result<()> {
    ...
    if !args.crates.is_empty() {
        ...
        for n in args.crates.iter() {
            ...
            info!("creating temp git repo in {patch_target_tmp_path:?}");
            git::init(&patch_target_tmp_path)?;
            info!("destroying git at {patch_target_path:?}");
            git::destroy(&patch_target_path)?;
            info!("Copying {patch_target_path:?} to {patch_target_tmp_folder:?}");
            copy_dir_all(&patch_target_path, &patch_target_tmp_folder)?;
            // copy(
            //     &patch_target_path,
...
fn copy_dir_all(src: impl AsRef<Path>, dst: impl AsRef<Path>) -> Result<()> {
    // Create a base destination directory for all contents of the source directory
    let base_dst = dst.as_ref().join(src.as_ref().file_name().unwrap());
    fs::create_dir_all(&base_dst)?;

    for entry in fs::read_dir(src.as_ref())? {
        let entry = entry?;
        let src_path = entry.path();
        let dst_path = base_dst.join(entry.file_name());
        if let Some(ugh) = entry.path().to_str() {
            if ugh.contains(".embuild") {
                continue;
            }
        }
        if entry.file_type()?.is_symlink() {
            ...
            // #[cfg(windows)]
            // fs::os::windows::fs::symlink(target, &dst_path)?;
            // #[cfg(unix)]
            // fs::os::unix::fs::symlink(target, &dst_path)?;
        } else if entry.file_type()?.is_dir() {
            copy_dir_all(src_path, &base_dst)?;
        } else {
